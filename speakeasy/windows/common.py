...
def normalize_dll_name(name):
    ...
    if name.lower().startswith(
        ("api-ms-win-crt", "vcruntime", "ucrtbased", "ucrtbase", "msvcr", "msvcp")
    ):
        ret = "msvcrt"
...
class PeFile(pefile.PE):
    ...
    def _patch_imports(self):
        ...
        for addr, imp in self.imports.items():
            ...
            tmp[offset : offset + self.ptr_size] = self.imp_id.to_bytes(
    ...
    def is_driver(self):
        ...
        if (
            ...
            == pefile.SUBSYSTEM_TYPE["IMAGE_SUBSYSTEM_NATIVE"]
            and self.ep == 0
        ):
            return True
...
class DecoyModule(PeFile):
    ...
    def __init__(
        self, path=None, data=None, fast_load=True, base=0, emu_path="", is_jitted=False
    ):
        self.image_size = 0
        self.ep = 0
    ...
    def get_memory_mapped_image(self, max_virtual_address=0x10000000, base=None):
        mmi = super(DecoyModule, self).get_memory_mapped_image(
            max_virtual_address, base
        )
...
class JitPeFile(object):
    ...
    def cast_section(self, offset=None):
        ...
        if offset is None:
            ...
            data = (
                pefile.Structure(self.basepe.__IMAGE_SECTION_HEADER_format__).sizeof()
                * b"\x00"
            )
        else:
    ...
    def update_image_size(self):
        ...
        """
        self.basepe.OPTIONAL_HEADER.SizeOfImage = (
            len(self.basepe.get_memory_mapped_image()) + PAGE_SIZE
        )
    ...
    def add_section(self, name, chars=0x40000040):
        ...
        hdr_size = pefile.Structure(
            self.basepe.__IMAGE_SECTION_HEADER_format__
        ).sizeof()
    ...
    def get_exports_size(self, name, exports):
        ...
        exp_size = pefile.Structure(
            self.basepe.__IMAGE_EXPORT_DIRECTORY_format__
        ).sizeof()
    ...
    def init_export_section(self, name, exports):
        ...
        dest_export_dir = self.basepe.__unpack_data__(
            ...
            pefile.Structure(self.basepe.__IMAGE_EXPORT_DIRECTORY_format__).sizeof()
            * b"\x00",  # noqa
            offset,
        )
    ...
    def init_text_section(self, names):
        ...
        if self.arch == _arch.ARCH_X86:
            ...
            for i in range(len(names)):
                pattern += (
                    b"\x48\x89\xFF\x90\x48\xC7\xC0" + i.to_bytes(4, "little") + b"\xc3"
                )
