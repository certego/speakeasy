...
class Msvcrt(api.ApiHandler):
    ...
    def _acmdln(self, ptr=0):
        ...
        if not ptr:
            cmdln = self.mem_alloc(
                len(_argv) + ptr_size, base=None, tag="api.msvcrt._acmdln"
            )
    ...
    @apihook(
        "__stdio_common_vfprintf", argc=e_arch.VAR_ARGS, conv=e_arch.CALL_CONV_CDECL
    )
    def __stdio_common_vfprintf(self, emu, argv, ctx={}):

        arch = emu.get_arch()
        if arch == e_arch.ARCH_AMD64:
            opts, stream, fmt, _, va_list = emu.get_func_argv(
                e_arch.CALL_CONV_CDECL, 5
            )[:5]
        else:
            opts, opts2, stream, fmt, _, va_list = emu.get_func_argv(
                e_arch.CALL_CONV_CDECL, 6
            )[:6]
    ...
    def _beginthreadex(self, emu, argv, ctx={}):
        ...
        handle, obj = self.create_thread(
            start_address, arglist, emu.get_current_process()
        )
