            ...
            if self.module_dir:
                ...
                if modules:
                    ...
                    self.cfg.update(
                        {
                            "modules": {
                                "module_directory_x86": self.module_dir,
        ...
        if args.no_mp:
            ...
            while True:
                if self.timeout and self.timeout < (time.time() - start_time):
                    evt.set()
                    self.logger.error(
                        "* Child process timeout reached after %d seconds"
                        % (self.timeout)
                    )
                    report = q.get(5)
                try:
...
def main():
    ...
    parser.add_argument(
        ...
        dest="target",
        required=False,
        help="Path to input file to emulate",
    )
    parser.add_argument(
        "-o",
        "--output",
        ...
        dest="output",
        required=False,
        help="Path to output file to save report",
    )
    parser.add_argument(
        "-p",
        "--params",
    ...
    parser.add_argument(
        ...
        dest="config",
        required=False,
        help="Path to emulator config file",
    )
    parser.add_argument(
        "-m",
        "--mem-tracing",
